...
export default function ModelsPage() {
  ...
  return (
    <div className="p-6">
      {/* Always-visible Add Model card */}
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base">Add Model Manually</CardTitle>
              <CardDescription>Type a model ID to add it to a provider</CardDescription>
            </div>
            <Button size="sm" variant="outline" onClick={() => setAddingModel("__standalone__")}>
              <Plus className="mr-2 h-4 w-4" />
              Add Model
            </Button>
          </div>
          {addingModel === "__standalone__" && (
            <div className="mt-4 flex items-center gap-2">
              <select
                className="h-8 rounded-md border border-input bg-background px-3 text-sm"
                id="standalone-provider"
                defaultValue="132e4aa2-c151-41f0-928e-4a9e683267ec"
              >
                <option value="132e4aa2-c151-41f0-928e-4a9e683267ec">Github Copilot</option>
              </select>
              <Input
                id="standalone-model-id"
                placeholder="model-id (e.g. gpt-4.1)"
                className="h-8 w-48 text-sm"
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    const provId = (document.getElementById("standalone-provider") as HTMLSelectElement)?.value;
                    const modelId = (document.getElementById("standalone-model-id") as HTMLInputElement)?.value?.trim();
                    if (provId && modelId) {
                      setAddingModel(null);
                      fetch("/api/admin/models/fetch", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ providerId: provId, manualModelIds: [modelId] }),
                      }).then(r => r.json()).then(data => {
                        if (data.error) toast({ title: "Failed to add model", description: data.error, variant: "destructive" });
                        else { toast({ title: "Model added" }); fetchModels(); }
                      }).catch(() => toast({ title: "Failed to add model", variant: "destructive" }));
                    }
                  }
